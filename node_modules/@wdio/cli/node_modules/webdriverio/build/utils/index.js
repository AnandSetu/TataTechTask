 ...
 */
export function elementPromiseHandler(handle, shadowRootManager, shadowRootId) {
    return (el) => {
...
export function transformClassicToBidiSelector(using, value) {
 ...
 */
export async function findDeepElement(selector) {
    const browser = getBrowserObject(this);
    const shadowRootManager = getShadowRootManager(browser);
    const handle = await browser.getWindowHandle();
    const shadowRoots = shadowRootManager.getShadowElementsByContextId(handle, this.elementId);
    ...
    const deepElementResult = (await Promise.all([
        ...
        browser.browsingContextLocateNodes({
            ...
            maxNodeCount: 1,
            ...(this.elementId
                ? { startNodes: [{ sharedId: this.elementId }] }
                : {})
        }).then((result) => {
         ...
         */
        ...shadowRoots.map((shadowRootNodeId) => browser.findElementFromShadowRoot(shadowRootNodeId, using, value).then(elementPromiseHandler(handle, shadowRootManager), elementPromiseHandler(handle, shadowRootManager)))
    ])).filter(Boolean);
    if (deepElementResult.length === 0) {
        return new Error(`Couldn't find element with selector "${selector}"`);
    }
    return deepElementResult[0];
}
/**
 * Parallel look up of a selector within multiple shadow roots
 * @param this WebdriverIO Browser or Element instance
 ...
 * @returns a list of shadow root ids with their corresponding matches or undefined if not found
 */
export async function findDeepElements(selector) {
    const browser = getBrowserObject(this);
    const shadowRootManager = getShadowRootManager(browser);
    const handle = await browser.getWindowHandle();
    const shadowRoots = shadowRootManager.getShadowElementsByContextId(handle, this.elementId);
    ...
    const deepElementResult = (await Promise.all([
         ...
         * fetch through all shadow roots
         */
        ...shadowRoots.map((shadowRootNodeId) => browser.findElementsFromShadowRoot(shadowRootNodeId, using, value).then(elementPromiseHandler(handle, shadowRootManager), elementPromiseHandler(handle, shadowRootManager)))
    ])).filter(Boolean);
