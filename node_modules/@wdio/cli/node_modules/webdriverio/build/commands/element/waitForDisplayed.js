 ...
 * @type utility
 */
export async function waitForDisplayed({ timeout = this.options.waitforTimeout, interval = this.options.waitforInterval, reverse = false, withinViewport = false, timeoutMsg = `element ("${this.selector}") still ${reverse ? '' : 'not '}displayed${withinViewport ? ' within viewport' : ''} after ${timeout}ms`, } = {}) {
    return this.waitUntil(async () => reverse !== await this.isDisplayed({ withinViewport }), { timeout, interval, timeoutMsg });
}
